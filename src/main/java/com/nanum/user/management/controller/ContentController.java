...
public class ContentController implements ResponseSupport {
    ...
    @GetMapping
    @Operation(summary = "게시글 목록 조회", description = "게시글 유형(ContentType: NOTICE, FAQ 등)에 따라 등록된 게시글 목록을 페이징하여 조회합니다. 키워드 검색이 가능합니다.")
    public ResponseEntity<ApiResponse<Page<ContentDTO.Response>>> getContents(
            @RequestParam ContentType type,
            @RequestParam(name = "site_cd") String siteCd,
            @RequestParam(required = false) String keyword,
            @PageableDefault(size = 5) Pageable pageable) {
        return success(contentService.getContents(type, siteCd, keyword, pageable));
    }
